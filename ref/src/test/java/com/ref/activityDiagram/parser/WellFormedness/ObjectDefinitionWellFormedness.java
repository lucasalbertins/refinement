...
public class ObjectDefinitionWellFormedness {
	 public static IActivityDiagram ad,ad2,ad3;
		...
		public static void getDiagram() throws  Exception,ProjectNotFoundException {
			...
			projectAccessor.open("src/test/resources/activityDiagram/submitData2UnspecifiedType.asta");
			INamedElement[] findElements2 = findElements(projectAccessor);
			ad2 = (IActivityDiagram) findElements2[0];
			
			
			projectAccessor.open("src/test/resources/activityDiagram/submitData2NoTypeDefinition.asta");
		...
		@Test
		public void testUnspecifiedType() throws FileNotFoundException, UnsupportedEncodingException, FDRException, ParsingException, IOException {
			try {
				CheckingProgressBar progressBar = new CheckingProgressBar();
				progressBar.setNewTitle("Checking deadlock");
				progressBar.setAssertion(0);
				...
				assertEquals(e.getMessage(),"Unspecified data type (size)");
			}
			
		}
		@Test
		public void testNoTypeDef() throws FileNotFoundException, UnsupportedEncodingException, FDRException, ParsingException, IOException {
			try {
				CheckingProgressBar progressBar = new CheckingProgressBar();
				progressBar.setNewTitle("Checking deadlock");
				progressBar.setAssertion(0);
				...
				assertEquals(e.getMessage(),"No data types in the object nodes");	
			} 
